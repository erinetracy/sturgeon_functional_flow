...
# Currently using 2007-2017 predictor data only
...
plot_partial_dependence_both <- function(model_HMC, model_VON, 
  ...
  var_labels <- c(
    "FA_pulse_dif"   = "Fall Pulse Difference",
    "Wet_BFL_Mag_50" = "Wet Season Median Magnitude",
    "Wet_Tim"        = "Wet Season Timing",
    "SP_Mag"         = "Spring Recession Magnitude",
    "SP_ROC"         = "Spring Rate of Change",
    "SP_Dur"         = "Spring Duration",
    "Peak_Fre_2"     = "Peak Flow Frequency"
  )
  
  make_row <- function(model) {
    top3 <- summary(model, plotit = FALSE) %>%
    ...
    plots <- lapply(top3, function(var) {
      pd_sub <- pd_all %>% filter(predictor == var)
      ggplot(pd_sub, aes(x = .data[[var]], y = fitted)) +
        geom_line(color = "steelblue", linewidth = 0.8) +
  ...
  row_HMC <- make_row(model_HMC)
  row_VON <- make_row(model_VON)
...
# Run one combined figure per response variable
plot_partial_dependence_both(brt_route_HMC,    brt_route_VON,
                             "Probability of Delta Route",       family = "bernoulli")

plot_partial_dependence_both(brt_success_HMC,  brt_success_VON,
                             "Probability of Migration Success", family = "bernoulli")

plot_partial_dependence_both(brt_duration_HMC, brt_duration_VON,
                             "Migration duration (days)",        family = "gaussian")
# =============================================================================
# 17. SAVE WORKSPACE
# =============================================================================

save.image(file = "gs_analysis_workspace.RData")
